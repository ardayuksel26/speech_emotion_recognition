...
API_URL = "http://127.0.0.1:5000/api/predict_mastermind"
...
def run_test():
        ...
        for wav_path in wav_files:
            try:
                ...
                if response.status_code == 200:
                    ...
                    log_msg = f"[Detay] {os.path.basename(wav_path)} | Gerçek Sınıf: {emo.upper():<6} | Üst Akıl Kararı: {pred_emotion.upper():<6} | RF_Sad_Skoru: {rf_sad}"
                    print(f"    {log_msg}")
    ...
    with open(REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("================================================================================\n")
        f.write(" 👑 THE MASTERMIND (ÜST AKIL) - AKADEMİK BENCHMARK RAPORU (TEST SET)\n")
        f.write("================================================================================\n")
    ...
    plt.title(f"Üst Akıl (Mastermind) Test Performans Değerleri\nAccuracy: %{acc*100:.1f}")
    plt.ylim(0, 1.05)
